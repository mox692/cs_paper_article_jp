...
CとC++はすでに未定義の動作が多く、コンパイラの最適化が暴走するような言語の隅っこで、ユーザーは迷子にならないようにしなければならない、さもないと。もう1つあっても害はないだろう？
既存のコンパイラやライブラリは、スレッドを無視して書かれており、無茶なプログラムを任意の方法で壊してしまう。すべての問題を発見して修正するのはあまりにも難しい、あるいはそう主張する。しかし、それらの修正されていないコンパイラやライブラリが、リラックスしたアトミックにどう対処するつもりなのかは不明である。
本当に自分のやっていることが分かっていて、未定義の動作を避けたいプログラマーは、relaxed atomicsを使うことができます。
レースセマンティクスを未定義のままにしておくと、実装がレースを検出・診断して実行を停止することができます。
